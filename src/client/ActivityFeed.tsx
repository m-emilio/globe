    ...
    <div className="activity-floating">
      <button
        ...
        className={`activity-launcher ${open ? "activity-launcher-open" : ""} ${
          !isSocketConnected ? "activity-launcher-offline" : ""
        }`}
        onClick={onToggle}
        aria-controls="live-feed-menu"
        aria-expanded={open}
        title={isSocketConnected ? "Live visitor feed" : "Feed reconnecting…"}
      >
        <span
          className={`pulse-dot ${!isSocketConnected ? "pulse-dot-offline" : ""} ${
            isPaused ? "pulse-dot-paused" : ""
          }`}
        />
        <span className="launcher-copy">
          <span className="launcher-label">Live Feed</span>
          <span className="launcher-meta">
            {isSocketConnected ? `${counter} online` : "reconnecting"}
            {isPaused ? " · paused" : ""}
            {leaveCount > 0 ? ` · ${leaveCount} left` : ""}
          </span>
        </span>
          ...
          <div className="activity-filter" role="group" aria-label="Filter activity">
            {(["all", "connect", "disconnect"] as ActivityFilter[]).map((f) => (
              ...
              >
                ...
                <span className="activity-filter-count">{filterCounts[f]}</span>
              </button>
            ))}
          </div>
                  ...
                  >
                    ...
                    <div className="activity-details">
                      ...
                      >
                        ...
                        {sessionLabel ? (
                          <span className="activity-session"> · {sessionLabel}</span>
                        ) : null}
                      </span>
                      {showMeta && (
